...
- [ ] The persistent sidebar navigates **Conversations → Agents → About**; switching views keeps the conversation navigator state (conversation list, selected conversation) intact and never cancels a conversation run.
- [ ] On a narrow window (or a mobile viewport) the sidebar collapses into a burger drawer: the toggle has `aria-expanded`, opening moves focus inside, Tab cycles within it, **Escape** closes it and returns focus to the toggle.
...
- Minimal web UI: React/Vite workbench with a responsive app shell (sidebar Conversations → Agents roster → About, mobile burger drawer), public auth probe, in-memory bearer-token entry, and the Conversation surface over the gateway Conversation API: conversation list/create by first-message raw text, C1/runnable-roster-gated attach (all members locally runnable), a raw-text follow-up composer, an immutable whole-history timeline plus scoped live SSE only after a successful attach, and lifecycle controls (Archive with an explicit confirmation, Reopen on archived inspection) that re-read the manifest and re-run the attach gate after a lifecycle action or event. On the active surface, live approval requests render as accessible non-modal cards (Confirm / Cancel, or a labeled input for select/input) submitting exactly-one responses, and each member has an Abort action for that conversation × agent run; read-only/archived inspection shows neither. Rejected conversations stay visibly read-only inspection with no composer or live stream. The browser never scans or resolves `@text` — mentions are server-authoritative. Persistent navigation beyond the conversation deep links and read-only vault/graph navigation remain separately gated; About is read-only connection/status only. No model or configuration controls.
- Conversation handoff loop (C5): the gated `conversation_handoff(to, text)` agent tool, available only inside broker-spawned Conversation runs, lets a steward-dispatched root lead REQUEST the initial handoff gate (a live `confirm` approval card confirmed through the existing Conversation approve route; nothing is dispatched, no audience changes, and no budget is consumed before confirmation) and lets child stages of an approved workflow hand off again within the finite per-workflow budget (depth 3 / edges 8 / rework 2) with no further per-handoff gate. Accepted edges are durable, replay-correlated handoff events; approvals stay live-only, abort stays one direct active conversation × agent run, and there is no hidden task bridge, queue, retry, reroute, fallback, or auto-approval. Workbench presentation of handoff-gate cards and live stage events (C5-4) remains separately gated.
