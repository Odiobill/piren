...
- [ ] Selecting an archived conversation or one with a member that is not locally runnable is rejected by the attach gate: it opens as visibly **read-only inspection** (history only, no composer, no live stream) with a non-secret explanation.
