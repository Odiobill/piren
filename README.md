...
- Steward alerts: durable alert files in the vault with an explicit Workbench close lifecycle, bounded closed-alert archive foundations, and an opt-in best-effort delivery mirror.
- Web workbench: React/Vite app with a Dashboard default page (local-policy agent roster, single-agent and peer Conversation start), an immutable timeline with scoped live streaming, approval and abort controls, confirmed archive/reopen lifecycle with read-only inspection of archived Conversations, a read-only Vault Explorer with safe fail-closed links, Steward Alerts with explicit close lifecycle, and a typed Settings page with write-only transport tokens and typed Scheduler class scopes. Bearer token in memory only, no live-session model or thinking controls, and no browser persistence.
...
Piren 0.2.5 is the current stable release on npm. Update an existing global install with:
