...
- [ ] The persistent sidebar navigates **Rooms → Agents → About**; switching views keeps the room navigator state (rooms list, selected room) intact and never cancels a room run.
...
- [ ] The roster lists every vault-defined agent; offline agents are labelled **Offline**, disabled, and explained as local policy (not a live probe); creating a room with online participants succeeds; a direct API POST with an offline participant is rejected with 400.
- [ ] Selecting a room shows its immutable timeline: historic events render chronologically, then the live stream updates them; the status distinguishes Live, Connecting, and Disconnected, and after a disconnect the timeline re-reads the whole history and re-subscribes (a Reconnect button appears if it stays down).
- [ ] Timeline entries are immutable (steward message, agent message, run started/finished/cancelled, approval-requested markers) and never editable; unreadable stream frames show as clearly non-authoritative markers.
- [ ] The "Coming next" card lists the remaining gated slices (dispatch, approvals/abort, accessibility completion); no chat/vault API calls are made by the shell.
...
- Minimal web UI: React/Vite workbench with a responsive app shell (sidebar Rooms → Agents roster → About, mobile burger drawer), public auth probe, in-memory bearer-token entry, a room navigator (room list/create/select with a local-policy agent roster where offline agents are visible but disabled), and an inspectable immutable room timeline (historic events plus live SSE with reconnect re-read, no cache); dispatch, direct chat, and approval/abort controls arrive in later separately-gated slices; About is read-only connection/status only. No model or configuration controls.
