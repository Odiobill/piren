...
- Minimal web UI: React/Vite workbench with a responsive app shell (sidebar Conversations → Agents roster → About, mobile burger drawer), public auth probe, in-memory bearer-token entry, and the Conversation surface over the gateway Conversation API: conversation list/create by first-message raw text, C1/runnable-roster-gated attach (all members locally runnable), a raw-text follow-up composer, and an immutable whole-history timeline plus scoped live SSE only after a successful attach; rejected conversations stay visibly read-only inspection with no composer or live stream. The browser never scans or resolves `@text` — mentions are server-authoritative. Conversation approval/abort controls and persistent navigation arrive in later separately-gated slices; About is read-only connection/status only. No model or configuration controls.
