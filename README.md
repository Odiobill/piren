...
- [ ] The **Dashboard** is the default page: it shows the local-policy roster (offline means not runnable on this installation, never presence) and the conversation list, starts a new Conversation only from an explicit steward agent selection with exactly `{agent}`, disables the start control while that request is busy, keeps errors visible, and retries only on a fresh explicit action. The persistent sidebar is the conversation switcher plus the **Dashboard** nav item; there is no redundant standalone Conversations nav link and no browser-local draft entry. Switching views keeps the conversation navigator state (conversation list, selected conversation) intact and never cancels a conversation run.
