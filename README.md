...
- Steward alerts: durable alert files in the vault with a read-only Workbench surface, archive lifecycle, and an opt-in best-effort delivery mirror.
- Web workbench: React/Vite app with a Dashboard default page (local-policy agent roster, single-agent and peer Conversation start), an immutable timeline with scoped live streaming, approval and abort controls, confirmed archive/reopen lifecycle with read-only inspection of archived Conversations, a read-only Vault Explorer with safe fail-closed links, Steward Alerts with archive lifecycle, and a typed Settings page with write-only transport tokens and typed Scheduler class scopes. Bearer token in memory only, no live-session model or thinking controls, and no browser persistence.
