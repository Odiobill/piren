...
Current verification baseline: 99 test files, 1377 tests, `npm run typecheck`, `npm run build`, and `npm run smoke` passing. `npm run clean-install:check` packs the local source into a tarball and installs it in an isolated HOME (no `github:` fetch in the normal path).
