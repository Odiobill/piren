...
- [ ] The **Dashboard** is the default page: it shows the local-policy roster as agent cards (offline means not runnable on this installation, never presence), starts a new Conversation only from an explicit steward agent selection with exactly `{agent}`, disables the start control while that request is busy and shows a truthful reduced-motion-safe pending line that never claims the agent is live, keeps errors visible, and retries only on a fresh explicit action. The persistent sidebar is the sole conversation switcher plus the **Dashboard** nav item; there is no duplicate Dashboard conversation list, no redundant standalone Conversations nav link, and no browser-local draft entry. Switching views keeps the conversation navigator state (conversation list, selected conversation) intact and never cancels a conversation run.
