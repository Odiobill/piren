...
- [ ] Selecting a conversation with all members locally runnable attaches it as the active surface: the immutable whole-history timeline renders chronologically, then the live stream updates it; the status distinguishes Live, Connecting, and Disconnected, and after a disconnect the timeline re-reads the whole history and re-subscribes (a Reconnect button appears if it stays down).
- [ ] Selecting a conversation with a member that is not locally runnable is rejected by the attach gate: it opens as visibly **read-only inspection** (history only, no composer, no live stream) with a non-secret explanation.
