...
describe("app shell source surface (static)", () => {
  ...
  it("the shell never calls chat endpoints, uses no storage, and stays write-scoped to the T1 assign task", async () => {
    const sources = await readSourceFiles();
    ...
    // transport. T1 (2026-08-21) additionally authorizes exactly ONE typed
    // Assign-task reference to /api/vault/inbox, in api.ts only, over the
    // existing authenticated route — never a generic vault write/editor.
    // Chat, native SSE, storage, and any other vault route stay forbidden.
    for (const [name, content] of sources) {
      ...
      expect(content, `${name} must not reference vault graph routes`).not.toContain("/api/vault/graph");
      if (name === "api.ts") {
        expect(content.match(/\/api\/vault\/inbox/g)?.length ?? 0).toBe(1);
