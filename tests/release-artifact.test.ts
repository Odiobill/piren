 ...
 * `0.2.0-internal.12` is the steward-authorized unpublished local tarball-only
...
describe("scoped @odiobill/piren releases (0.2.0-internal.12 local tarball; 0.1.7/0.1.6/0.1.5/0.1.4 OIDC; 0.1.3 bootstrap)", () => {
  it("package.json name is the scoped @odiobill/piren identity", () => {
    const pkg = JSON.parse(read("package.json")) as { name: string };
    expect(pkg.name).toBe("@odiobill/piren");
  });

  it("package.json version is the unpublished 0.2.0-internal.12 local pilot", () => {
    const pkg = JSON.parse(read("package.json")) as { version: string };
    expect(pkg.version).toBe("0.2.0-internal.12");
  });
  ...
  it("readVersion reports 0.2.0-internal.12 from the real package.json", () => {
    expect(readVersion(join(repoRoot, "package.json"))).toBe("0.2.0-internal.12");
  });
  ...
  it("package-lock.json name and version agree with package.json", () => {
    ...
    expect(lock.version).toBe("0.2.0-internal.12");
    expect(lock.packages?.[""]?.name).toBe("@odiobill/piren");
    expect(lock.packages?.[""]?.version).toBe("0.2.0-internal.12");
  });
