 ...
 * `0.2.0` is the final release prepared locally; it is not yet published
 * until the `v0.2.0` tag is pushed and the OIDC workflow verifies/publishes
 * the exact artifact. It must not be asserted as already on npm or tagged. These guards keep package metadata,
...
describe("scoped @odiobill/piren releases (0.2.0 prepared locally; 0.1.7/0.1.6/0.1.5/0.1.4 OIDC; 0.1.3 bootstrap)", () => {
  it("package.json name is the scoped @odiobill/piren identity", () => {
    const pkg = JSON.parse(read("package.json")) as { name: string };
    expect(pkg.name).toBe("@odiobill/piren");
  });

  it("package.json version is the prepared 0.2.0 final release", () => {
  ...
  it("readVersion reports 0.2.0 from the real package.json", () => {
    expect(readVersion(join(repoRoot, "package.json"))).toBe("0.2.0");
  });
  ...
  it("CHANGELOG has a dated [0.2.0] entry above 0.1.7", () => {
    ...
    expect(section).toMatch(/## \[0\.2\.0\] - 2026-08-26/);
