...
export const SERVICE_TRANSPORTS = ["gateway", "telegram", "discord"] as const;
...
export function validateTransport(transport: string): ValidationResult {
  ...
  return { ok: false, message: `Unknown transport '${transport}'. Use one of: ${SERVICE_TRANSPORTS.join(", ")}.` };
}
...
export function generateTmuxLaunchScript(opts: GenerateServiceOptions): string {
  ...
  const startCmd = `${opts.pirenCommand} ${opts.transport} --vault-root ${opts.vaultRoot} --agent ${opts.agentName}`;
  return [
    "#!/bin/sh",
