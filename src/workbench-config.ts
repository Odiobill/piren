...
function defaultResolution(): WorkbenchConfigResolution {
 ...
 */
export function parseWorkbenchConfig(source: string | null): WorkbenchConfigResolution {
  if (source === null) return defaultResolution();
  ...
  if (document === null || document === undefined) return defaultResolution();
  if (!isPlainObject(document)) {
    return { ...defaultResolution(), warnings: [{ key: null, category: "malformed-yaml" }] };
  }
  const conversation = document["conversation"];
  if (conversation === undefined) return defaultResolution();
  ...
  if (raw === undefined) return defaultResolution();
...
export function workbenchRunDeadlineMs(resolution: WorkbenchConfigResolution): number {
 ...
 */
export function formatWorkbenchConfigWarning(warning: WorkbenchConfigWarning): string {
  const reason =
    warning.category === "malformed-yaml"
      ? "could not be parsed"
  ...
  return (
    `workbench.yml: ${keyPart}${reason} (${warning.category}); ` +
 ...
 */
export function createNodeWorkbenchConfigReader(vaultRoot: string): () => string | null {
  return () => {
    try {
