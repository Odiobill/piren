...
function defaultKnowledgeLifecycleConceptContent(): string {
  const today = new Date().toISOString().slice(0, 10);
  return [
    "---",
    "type: Concept",
...
function defaultOkfKnowledgeBundleConceptContent(): string {
  const today = new Date().toISOString().slice(0, 10);
  return [
    "---",
    "type: Concept",
...
export async function initVault(options: InitVaultOptions): Promise<InitVaultResult> {
  ...
  try {
    ...
    await writeNewFile(
      join(vaultRoot, "wiki", "concepts", "knowledge-lifecycle.md"),
      defaultKnowledgeLifecycleConceptContent(),
      force,
      created,
    );
    await writeNewFile(
      join(vaultRoot, "wiki", "concepts", "okf-knowledge-bundle.md"),
      defaultOkfKnowledgeBundleConceptContent(),
      force,
      created,
    );
  } catch (error) {
