 ...
 */
export function checkModelFallbackConfig(
  config: Record<string, unknown> | null,
  id: string = "model-fallback",
): DoctorCheck | null {
  if (config === null) return null;
  const modelBlock = config.model;
  if (typeof modelBlock !== "object" || modelBlock === null || Array.isArray(modelBlock)) return null;
  const modelRecord = modelBlock as Record<string, unknown>;
  if (!("fallback" in modelRecord)) return null;
  ...
  if (!parsed.present) return null; // unreachable after the "in" guard; defensive
  ...
  if (primaries.some((primary) => parsed.config.models.includes(primary))) {
    return {
      ...
      message: withWarnGuidance(
        "model.fallback.models includes the configured primary model.id; rotation would simply re-attempt the primary.",
        AUTHORITY_MODEL_FALLBACK,
        MODEL_FALLBACK_NEXT,
      ),
    };
  }
...
export function checkSchedulerAutomationConfig(config: LocalPirenConfig): DoctorCheck | null {
  ...
  if (resolved.legacyMasterGate === "gated") {
    return warn(
      "scheduler.enabled",
      "scheduler.enabled is present as a retired legacy gate; all automation classes resolve disabled (fail closed) until an operator-confirmed piren scheduler configure migration removes the retired key",
    );
  }
  let message =
    `scheduler: inbox_tasks: ${resolved.automation.inboxTasks}; ` +
  ...
  if (!hasExplicitAgentSelection && configuredVaultRoot !== undefined && runnableAgents.length !== 1) {
    ...
    try {
      ...
      for (const agent of enabledAgents) {
        ...
        if (modelFallbackCheck) checks.push(modelFallbackCheck);
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
