...
import { isValidAgentName, type RoomRunFailureKind, ROOM_RUN_FAILURE_KINDS } from "./rooms.js";
...
export const ROOM_HANDOFF_PROTOCOL_VERSION = 1 as const;
...
export const ROOM_HANDOFF_RESULT_STATUSES = ["ok", "rejected", "failed", "timed_out", "cancelled"] as const;
export type RoomHandoffResultStatus = (typeof ROOM_HANDOFF_RESULT_STATUSES)[number];

/**
 * The bounded, non-secret result rendered into the input response `value`.
 * The lead's tool result is derived only from this shape; it never carries
 * secrets, raw errors, room ids, or source identity.
 */
export type RoomHandoffResult =
  | { status: "ok"; reply: string }
  | { status: "rejected"; reason: string }
  | { status: "failed"; reason: string; failureKind: RoomRunFailureKind }
  | { status: "timed_out" }
  | { status: "cancelled" };

/** Outcome of parsing a reserved handoff input request envelope. */
export type HandoffRequestParse =
  | { ok: true; version: number; to: string; text: string }
  | { ok: false; reason: string };

/** Outcome of parsing a rendered handoff result value (extension side). */
export type HandoffResultParse =
  | { ok: true; version: number; result: RoomHandoffResult }
  | { ok: false; reason: string };
 ...
 */
export function isHandoffInputRequest(event: RpcEvent): boolean {
  return (
 ...
 */
export function parseHandoffInputRequest(event: RpcEvent): HandoffRequestParse {
 ...
 */
export function renderHandoffResultValue(result: RoomHandoffResult): string {
  const payload: Record<string, unknown> = { v: ROOM_HANDOFF_PROTOCOL_VERSION, status: result.status };
  if (result.status === "ok") {
...
export function isRoomMentionEnabled(env: Record<string, string | undefined>): boolean {
 ...
 */
export function renderHandoffRequestPlaceholder(to: string, text: string): string {
 ...
 */
export function validateRoomMentionArgs(to: string, text: string): string | null {
...
/** Outcome of mapping a raw input response value to a room_mention tool result. */
export type RoomMentionToolOutcome = { ok: true; reply: string } | { ok: false; error: string };
 ...
 */
export function resolveRoomMentionToolResult(value: string | undefined | null): RoomMentionToolOutcome {
 ...
 */
export function truncateHandoffReply(reply: string): string {
 ...
 */
export function parseHandoffResultValue(value: string): HandoffResultParse {
  let parsed: unknown;
  try {
  ...
  if (typeof status !== "string" || !(ROOM_HANDOFF_RESULT_STATUSES as readonly string[]).includes(status)) {
  ...
  if (status === "failed") {
    ...
    if (typeof parsed.failureKind !== "string" || !(ROOM_RUN_FAILURE_KINDS as readonly string[]).includes(parsed.failureKind)) {
      return { ok: false, reason: "handoff result failed status requires a valid failureKind" };
    }
    return {
      ok: true,
      version: ROOM_HANDOFF_PROTOCOL_VERSION,
      result: { status: "failed", reason: parsed.reason, failureKind: parsed.failureKind as RoomRunFailureKind },
    };
  }
  // timed_out | cancelled carry no extra fields.
  return { ok: true, version: ROOM_HANDOFF_PROTOCOL_VERSION, result: { status } as RoomHandoffResult };
}
