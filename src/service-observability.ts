...
// ---------------------------------------------------------------------------
// Snapshot model (exactly the accepted D2 contract vocabulary)
// ---------------------------------------------------------------------------
...
export const SERVICE_OBSERVATION_TARGETS = ["telegram", "discord", "scheduler"] as const;
...
export function systemdIsActiveArgv(target: ServiceObservationTarget): string[] {
  return ["systemctl", "--user", "is-active", `piren-${target}.service`];
}

export function tmuxHasSessionArgv(target: ServiceObservationTarget): string[] {
 ...
 */
export function classifySystemdIsActive(result: CommandRunResult): ServiceObservedState {
  if (result.signal !== null || result.exitCode === null) return "unknown";
  const out = result.stdout.trim();
  if (result.exitCode === 0 && out === "active") return "active";
 ...
 * code is `unknown`.
 */
export function classifyTmuxHasSession(result: CommandRunResult): ServiceObservedState {
  if (result.signal !== null || result.exitCode === null) return "unknown";
  if (result.exitCode === 0) return "active";
  if (result.exitCode === 1) return "inactive";
  return "unknown";
}
...
async function detectManagerKind(deps: ServiceObservationDeps): Promise<ServiceManagerKind> {
  try {
    const manager = await detectServiceManager(deps);
    if (manager === "systemd") return "systemd-user";
    if (manager === "tmux-cron") return "tmux-cron";
    return "unavailable";
  } catch {
...
async function observeTarget(
  deps: ServiceObservationDeps,
  manager: Exclude<ServiceManagerKind, "unavailable">,
  target: ServiceObservationTarget,
): Promise<ServiceObservedState> {
  try {
    const artifact: ServiceArtifactKind = manager === "systemd-user" ? "systemd-unit" : "tmux-launch-script";
    if (!(await deps.artifactExists(artifact, target))) return "not-installed";
 ...
 */
export async function observeServiceStatus(deps: ServiceObservationDeps): Promise<ServiceStatusSnapshot> {
  ...
  if (manager === "unavailable") {
    return {
      observedAt,
      manager,
      targets: SERVICE_OBSERVATION_TARGETS.map((target) => ({ target, state: "unavailable" as const })),
    };
  }
  const targets = await Promise.all(
    SERVICE_OBSERVATION_TARGETS.map(async (target) => ({ target, state: await observeTarget(deps, manager, target) })),
  );
...
function runCapture(argv: readonly string[]): Promise<CommandRunResult> {
 ...
 */
export function createLocalServiceObservationDeps(homeDir?: string): ServiceObservationDeps {
