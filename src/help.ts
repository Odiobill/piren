...
export const HELP_TOPICS: readonly CommandHelpTopic[] = [
  ...
  {
    command: "skill",
    short: "Manage vault skills: list, show, explain, create, move, promote, demote, conflicts, validate, staged local import (inactive), and explicit staged promotion (activates the skill).",
    flags: ["<list|show|explain|create|move|promote|demote|conflicts|validate>", "import <local-file.md> --staged [--name <slug>] [--force]", "staged <list|show> <name>", "staged promote <name> --to shared|group:<name>|agent:<name> [--force]", "<name>", "--agent <agent>", "--group <group>", "--scope shared|group:<name>|agent:<name>", "--from <scope>", "--to <scope>", "--force"],
  ...
  {
    command: "task",
    short: "Human-facing inbox task CLI: list, send, show, claim, complete, cancel, archive. No polling, no scheduler changes.",
    flags: ["<list|send|show|claim|complete|cancel|archive>", "<agent>", "<title>", "<path-or-id>", "--agent <agent>", "--body <vault-file>", "--result <vault-file>", "--priority normal|high|urgent", "--device <id>", "--yes"],
    example: 'piren task send dipu "Ship rc.4" --priority high && piren task archive --agent dipu  # preview terminal tasks; re-run with --yes to archive that exact preview',
  },
];
