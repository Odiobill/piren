...
export type TransportFallbackNotice =
  ...
  | { kind: "unavailable"; modelId: string; attempt: number }
  | {
      kind: "exhausted";
      lastModelId: string;
      category: "provider_error_other" | "provider_error_transient_exhausted";
...
export type TransportFallbackResult =
  | { status: "completed"; events: RpcEvent[] }
  | {
      status: "exhausted";
      events: RpcEvent[];
      lastModelId: string;
 ...
 * handoff re-prompt.
 */
export async function runTransportFallback(input: TransportFallbackRunInput): Promise<TransportFallbackResult> {
  ...
  let currentModelId: string | null = policy.primaryModelId;
  ...
  while (result === null) {
    ...
    if (plan.kind === "exhausted") {
      const notice: TransportFallbackNotice = {
        kind: "exhausted",
 ...
 */
export function renderTransportFallbackNotice(notice: TransportFallbackNotice): string {
  switch (notice.kind) {
    case "attempt":
