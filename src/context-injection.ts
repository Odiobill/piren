...
function parseMode(value: unknown): ContextInjectionMode | null {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
  return (CONTEXT_INJECTION_MODES as readonly string[]).includes(trimmed) ? (trimmed as ContextInjectionMode) : null;
}
...
export function resolveContextInjectionMode(options: ResolveContextInjectionModeOptions): ResolvedContextInjectionMode {
  ...
  let configMode: ContextInjectionMode = DEFAULT_CONTEXT_INJECTION_MODE;
  ...
  if (block !== undefined) {
    if (!block || typeof block !== "object" || Array.isArray(block)) {
      ...
      const parsed = parseMode((block as Record<string, unknown>)["mode"]);
      if (parsed !== null) {
        configMode = parsed;
      } else {
        warnings.push(`Unknown context_injection.mode '${String((block as Record<string, unknown>)["mode"])}' in agent config (expected per_turn | session_start_only); falling back to ${DEFAULT_CONTEXT_INJECTION_MODE}.`);
      }
    }
  }
...
export function shouldInjectContext(options: ShouldInjectContextOptions): boolean {
  if (options.mode === "per_turn") return true;
