...
export const CLI_PRIORITIES = ["normal", "high", "urgent"] as const;
export type CliPriority = (typeof CLI_PRIORITIES)[number];

export function isValidCliPriority(value: string): value is CliPriority {
  return (CLI_PRIORITIES as readonly string[]).includes(value);
}
 ...
 */
export function isLikelyTaskPath(input: string): boolean {
 ...
 */
export function assertVaultContained(vaultRoot: string, absPath: string): void {
  ...
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(
      `Path resolves outside vault: ${absPath}`,
    );
  }
}
 ...
 */
export function assertInboxTaskRelPath(
  vaultRoot: string,
  relPath: string,
  ...
  if (
    ...
    throw new Error(
      "Task path must point to a Markdown task file directly under team/<agent>/inbox/.",
    );
  }
  const agentName = parts[1] ?? "";
  if (!AGENT_NAME_PATTERN.test(agentName)) {
    throw new Error(
      "Invalid agent name. Use lowercase kebab-case, for example 'piren' or 'research-agent'.",
    );
  }
...
// ---------------------------------------------------------------------------
// Frontmatter parsing (self-contained, mirrors inbox parsing)
// ---------------------------------------------------------------------------
...
function parseTaskFields(content: string, path: string): ParsedTaskFile {
 ...
 */
export async function resolveTaskIdOrPath(
  deps: TaskCliDeps,
  vaultRoot: string,
  input: string,
  agent?: string,
): Promise<{ path: string; agentName: string }> {
  ...
  const matches: { agentName: string; relPath: string }[] = [];
  ...
  if (matches.length > 1) {
    ...
    if (distinctAgents.size > 1) {
      throw new Error(
        `Task id '${input}' is ambiguous across agents (${[...distinctAgents].join(", ")}). Pass --agent <agent> to disambiguate.`,
      );
    }
...
export async function readTaskDetail(
  ...
  return {
    ...
    status: (fields.status as TaskStatus) ?? "pending",
...
export function formatTaskList(rows: TaskListRow[]): string {
  ...
  const lines: string[] = [];
  lines.push(
    `${pad("STATUS", statusWidth)}  ${pad("FROM", fromWidth)}  ${pad("TITLE", titleWidth)}  PATH`,
  );
  lines.push(
    `${"-".repeat(statusWidth)}  ${"-".repeat(fromWidth)}  ${"-".repeat(titleWidth)}  ${"-".repeat(4)}`,
  );
  for (const row of rows) {
    const title = row.title.length > titleWidth ? row.title.slice(0, titleWidth - 1) + "…" : row.title;
    lines.push(
      `${pad(row.status, statusWidth)}  ${pad(row.from, fromWidth)}  ${pad(title, titleWidth)}  ${row.path}`,
    );
  }
  return lines.join("\n");
}
...
export function formatTaskDetail(detail: TaskDetail): string {
  ...
  if (detail.created) lines.push(`Created: ${detail.created}`);
  if (detail.updated) lines.push(`Updated: ${detail.updated}`);
...
export function createRealTaskCliDeps(): TaskCliDeps {
  return {
