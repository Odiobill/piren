...
// ---------------------------------------------------------------------------
// S1a: scheduler class x agent local policy resolver
// ---------------------------------------------------------------------------
...
const AGENT_SCOPE_CLASS_FIELDS: Record<SchedulerAutomationClass, keyof ResolvedSchedulerAgentScope> = {
  inbox_tasks: "inboxTasks",
  agent_cron: "agentCron",
  script_cron: "scriptCron",
};

const AGENT_SCOPE_CLASS_LABELS: Record<SchedulerAutomationClass, string> = {
...
function failClosedClassScope(): string[] {
  return [];
}

/** Fail-closed scope for every class. */
function failClosedAllClassScopes(): ResolvedSchedulerAgentScope {
 ...
 */
function parseAgentScopeList(
  container: Record<string, unknown>,
  key: "allow" | "exclude",
  classKey: SchedulerAutomationClass,
  warnings: string[],
): { names: string[] | undefined; malformed: boolean } {
  const raw = container[key];
  if (isAbsentLike(raw)) return { names: undefined, malformed: false };
  if (
    !Array.isArray(raw) ||
    raw.some((entry) => typeof entry !== "string" || entry.trim() === "")
  ) {
    warnings.push(
      `scheduler.agent_scope.${classKey}.${key} has invalid ${configValueKind(raw)} content; ` +
  ...
  return { names: [...new Set(raw as string[])], malformed: false };
}
 ...
 */
export function resolveSchedulerAgentScope(
  raw: unknown,
  runnableAgents: string[],
): ResolveSchedulerAgentScopeResult {
  const warnings: string[] = [];
  if (isAbsentLike(raw)) return { scope: {}, warnings };
  if (!isPlainRecord(raw)) {
    warnings.push(
      `scheduler.agent_scope has invalid ${configValueKind(raw)} content; ` +
  ...
  const scope: ResolvedSchedulerAgentScope = {};
  ...
  for (const classKey of SCHEDULER_AUTOMATION_CLASSES) {
    ...
    if (isAbsentLike(rawClass)) continue;
    if (!isPlainRecord(rawClass)) {
      warnings.push(
        `scheduler.agent_scope.${classKey} has invalid ${configValueKind(rawClass)} content; ` +
          `excluding all agents for ${AGENT_SCOPE_CLASS_LABELS[classKey]} automation (fail closed).`,
      );
    ...
    if (unknownScopeKeys.length > 0) {
      warnings.push(
        `scheduler.agent_scope.${classKey} contains ${unknownScopeKeys.length} unrecognized key(s); ignoring them.`,
      );
    }
    ...
    if (allow.names === undefined && exclude.names === undefined) continue;
    ...
    for (const [key, names] of [
      ...
      if (unknownCount > 0) {
        warnings.push(
          `scheduler.agent_scope.${classKey}.${key} contains ${unknownCount} name(s) that are not locally enabled agents; ` +
    ...
    if (allowNames !== undefined) {
      ...
      if (excludeNames !== undefined) agents = agents.filter((name) => !excludeNames.includes(name));
  ...
  if (unknownClassKeys.length > 0) {
    warnings.push(
      `scheduler.agent_scope contains ${unknownClassKeys.length} unrecognized class key(s); ignoring them.`,
    );
  }
...
export function resolveSchedulerConfig(config: LocalPirenConfig): ResolvedSchedulerConfig {
  ...
  for (const warning of agentScopeResult.warnings) warnings.push(warning);
...
function formatStartupSummary(
  ...
  lines.push(
    `agent scope: ${scopePart("inbox_tasks", schedulerConfig.agentScope.inboxTasks)} ` +
