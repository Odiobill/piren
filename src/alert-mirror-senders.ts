 ...
 */
export function createAlertMirrorSenders(config: LocalPirenConfig, fetchImpl: typeof fetch = fetch): AlertMirrorSenders {
  ...
  if (telegramToken !== "") {
    ...
    senders.telegram = async (chatId: string, text: string) => {
  ...
  if (discordToken !== "") {
    ...
    senders.discord = async (channelId: string, text: string) => {
