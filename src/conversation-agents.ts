 ...
 */
export function buildConversationAgentsResponse(
  vaultAgents: readonly string[],
  runnableAgents: readonly string[],
  configuredModels?: Readonly<Record<string, string | null | undefined>>,
): ConversationAgentsResponse {
  const runnable = new Set(runnableAgents);
  const names = [...new Set(vaultAgents)].sort((a, b) => a.localeCompare(b));
  return {
    agents: names.map((name) => {
      const entry: ConversationAgentEntry = { name, online: runnable.has(name) };
 ...
 * resulting map.
 */
export async function projectConfiguredAgentModels(
  vaultRoot: string | undefined,
  vaultAgents: readonly string[],
  deps: ProjectConfiguredAgentModelsDeps = {},
