...
// ---------------------------------------------------------------------------
// Bounded prompt for one claimed agent-mode cron job
// ---------------------------------------------------------------------------
 ...
 */
export function buildClaimedCronJobPrompt(options: BuildClaimedCronJobPromptOptions): string {
...
// ---------------------------------------------------------------------------
// Claimed cron job path validation
// ---------------------------------------------------------------------------
 ...
 */
export function parseClaimedCronJobPath(options: ParseClaimedCronJobPathOptions): ClaimedCronJobPathInfo {
...
// ---------------------------------------------------------------------------
// Execution seam (structurally identical to the inbox executor's runner)
// ---------------------------------------------------------------------------
 ...
 */
export async function executeClaimedAgentCronJob(
  options: ExecuteClaimedAgentCronJobOptions,
): Promise<ExecuteClaimedAgentCronJobResult> {
  ...
  if (job.mode !== "agent") {
    throw new Error(
      `Claimed cron job is not in agent mode (mode: ${job.mode}): ${info.claimedJobPath}. Script-mode jobs use executeScriptCronJob.`,
    );
  }
  if (job.agent !== options.agentName) {
    throw new Error(
      `Claimed cron job frontmatter agent '${job.agent}' does not match selected agent '${options.agentName}': ${info.claimedJobPath}`,
    );
  }
  ...
  const status: CronRunStatus = ok ? "completed" : "failed";
  ...
  const result: ExecuteClaimedAgentCronJobResult = {
    agentName: info.agentName,
    deviceId: info.deviceId,
    jobId: job.id,
    claimedJobPath: info.claimedJobPath,
