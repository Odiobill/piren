...
async function main() {
  ...
  try {
    ...
    const mirroredSends: Array<{ id: string; text: string }> = [];
    const mirrorPi = fakePi();
    await extension(mirrorPi as any, {
    ...
    if (mirrored.isError) throw new Error(`alert-mirror flag_steward smoke failed: ${mirrored.content[0].text}`);
    const mirroredText: string = mirrored.content[0].text;
    ...
    const mirrorStatusNotifications: string[] = [];
    await mirrorPi.commands.piren_status.handler([], { ui: { notify: (m: string) => mirrorStatusNotifications.push(m) } });
    ...
    if (triggerResult.isError) throw new Error(`self_improvement_trigger_check smoke failed: ${triggerResult.content[0].text}`);
    ...
    const nudgeNotifications: Array<{ message: string; level: string }> = [];
    await nudgeHandler(
      { message: { role: "user", content: "Please don't use a hidden memory store." } },
      { ui: { notify: (message: string, level: string) => nudgeNotifications.push({ message, level }) } },
    );
    await nudgeHandler(
      { message: { role: "user", content: "No worries, all good." } },
      { ui: { notify: (message: string, level: string) => nudgeNotifications.push({ message, level }) } },
    );
    ...
    try {
      ...
      if (lastType !== "agent_settled") throw new Error(`RPC stream did not end with agent_settled: ${lastType}`);
    ...
    try {
      ...
      if (!entryMatch?.[1]) throw new Error("built index.html did not reference an entry asset");
      const entryRes = await fetch(`http://${staticHandle.hostname}:${staticHandle.port}${entryMatch[1]}`);
      if (entryRes.status !== 200) throw new Error(`entry asset HTTP ${entryRes.status}`);
