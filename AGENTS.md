...
- Clean-install validation now also guards the install lifecycle: `package.json` does not define `prepare`, so `npm install -g github:Odiobill/piren` does not compile TypeScript on the target machine. GitHub installs use the committed `dist/` release artifacts. `prepack` still rebuilds `dist/` for tarball creation.
