...
- `.github/workflows/release-publish.yml` (ADR-0033 P1) is the ONLY workflow that publishes to npm: tag-only (`v*`), gated by the protected `npm-production` GitHub Environment, minimal permissions (`contents: read` + `id-token: write`), no token/secret, runs the four quality gates, checks tag/package version agreement, builds one explicit tarball (`npm pack`), validates+installs that exact tarball via the clean-install machinery with the CI-only fake pi shim, then publishes it with `npm publish --provenance --access public --tag latest`. `release-verify.yml` stays verification-only.
