...
- `scripts/clean-install-check.ts` wires ADR-0033 Slice R1: the DEFAULT path packs the exact local source via `npm pack`, validates the packed surface, then installs that tarball into an isolated HOME/prefix (no `github:` fetch, no `--install-links` in the normal path, not blocked by `EALLOWGIT`). `resolveInstallSpec(args)` returns `{kind:"packed-tarball",source:"local"}` by default or `{kind:"explicit",spec}` when a positional spec is passed. `runPackedCleanInstallCheck` delegates the install + dist/binary/Pi checks to the existing `runCleanInstallCheck` (which now adds `--install-links` only when `needsInstallLinks(spec)` is true for git/github specs). CLI: `npm run clean-install:check [-- <spec>] [--allow-scripts] [--keep]`. Exits non-zero on failure so it is CI-safe.
- Tests: `tests/clean-install.test.ts` (7 tests) + `tests/clean-install-pack.test.ts` (28 tests: resolveInstallSpec default, packed-artifact surface check incl. a stable docs file, `npm pack --json` parsing, `buildLocalTarball` with injected deps, `needsInstallLinks`, source-aware formatting, `engines.node`, and tarball-cleanup regression over failure/success/--keep/thrown-install paths via an injected `InstallRunner`). The full real packed-tarball install is exercised manually via `npm run clean-install:check`, not in the unit suite.
