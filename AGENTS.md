...
- Tests: `tests/clean-install.test.ts` (7 tests) + `tests/clean-install-pack.test.ts` (22 tests: resolveInstallSpec default, packed-artifact surface check, `npm pack --json` parsing, `buildLocalTarball` with injected deps, `needsInstallLinks`, source-aware formatting, `engines.node`). The full real packed-tarball install is exercised manually via `npm run clean-install:check`, not in the unit suite.
