...
- `src/scheduler-planner.ts` is the pure scheduler planner core, callable directly from tests without Pi auth. It exports `planSchedulerTick` (takes enabled agents, active devices, pending tasks, due cron jobs, and `now`; returns proposed claim attempts sorted by device priority). Composes with the existing `selectOwningDevice` and `listActiveDevices` from `src/cron.ts`.
- `src/scheduler-executor.ts` exports the bounded executor core: `buildInboxExecutor` and `buildCronExecutor` (pure helpers that construct the Vi-style bounded prompt for one already-claimed item, validated for vault-relative path safety and unclaimed rejection) plus `createSchedulerExecutors` (adapts to `piren ask` for live execution). Agent-mode claims receive a claim-scoped prompt that reads the claimed file, executes it, writes status/result, and stops. Script-mode cron jobs reuse the existing `executeScriptCronJob` from `src/cron.ts` with no LLM.
- `src/scheduler-loop.ts` exports the pure tick-execution core `planAndExecuteOnce` (refreshes heartbeats, plans, attempts atomic claims, executes at most one successfully claimed item) and the opt-in `runSchedulerLoop` (repeats ticks at the configured `poll_interval_seconds` until SIGINT/SIGTERM). Local config is read from the `scheduler:` block in `~/.config/piren/config.yml`. Defaults: 30s poll, 300s stale, 1 max concurrent agent.
- `src/scheduler-service.ts` exports `resolveSchedulerConfig` (reads local `~/.config/piren/config.yml` and resolves `poll_interval_seconds`, `stale_after_seconds`, `max_concurrent_agents`, and `device_id` with deterministic defaults and warnings).
- `piren scheduler --dry-run` prints proposed claims without claiming or spawning; `piren scheduler --once` runs one live tick; `piren scheduler` runs the opt-in loop. All three are LLM-free on the planning tick. CLI wired in `src/cli.ts` and `src/parse-args.ts`.
...
- Tests: `tests/scheduler-planner.test.ts` (12 tests), `tests/scheduler-executor.test.ts` (22 tests), `tests/scheduler-loop.test.ts` (28 tests), `tests/scheduler-service.test.ts` (8 tests), `tests/cli-scheduler.test.ts` (5 tests), `tests/cli-service.test.ts` (3 tests). Service lifecycle tests extended for scheduler in `tests/service-lifecycle.test.ts` (+12 → 50), `tests/service-status-yaml.test.ts` (+1 → 6), `tests/doctor-service.test.ts` (+2 → 9). Smoke covers all four scheduler layers.
