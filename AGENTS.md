...
- ADR-0024 first and second slices are implemented in `src/self-improvement.ts`: Hermes-style correction trigger heuristics (strong/weak/negative regex patterns plus directive-word filtering), visible artifact suggestions retargeted at `project_append_log`, `skill_candidate_write`, `decision_record`, `wiki_update_concept`, and `wiki_update_entity`, and the read-only `self_improvement_trigger_check(message)` extension tool. It is advisory only: no hidden memory store, no SQLite, and no silent background writes. Opt-in auto-nudge wiring is also implemented: `resolveAutoNudgeConfig({env, config})` (env `PIREN_AUTO_NUDGE` overrides agent-local `self_improvement.auto_nudge`, default OFF) and `buildAutoNudgeNotification(message)` (pure; returns null on no-trigger, an inspectable notification otherwise). When enabled, the extension installs a `message_end` handler that emits an advisory `ctx.ui.notify` on detected user corrections and never writes to the vault on its own; the handler is best-effort and swallows internal errors so it can never abort a turn.
- Tests: `tests/knowledge.test.ts` (17 tests), `tests/self-improvement.test.ts` (12 tests: correction detection, artifact suggestions, auto-nudge config resolution, auto-nudge notification builder), `tests/pi-extension.test.ts` (extension coverage for all knowledge, wiki, and trigger tools, the opt-in auto-nudge `message_end` handler, context prompt assertions). Smoke covers all six knowledge/self-improvement tools plus wiki concept/entity writes, the trigger-check nudge, and the opt-in auto-nudge wiring (default-off plus `PIREN_AUTO_NUDGE=1` fires once on a correction, suppressed on a negative pattern).
