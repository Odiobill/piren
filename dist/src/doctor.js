...
import { resolveSchedulerConfig, SCHEDULER_AUTOMATION_CLASSES, } from "./scheduler-loop.js";
/**
 ...
 */
function withWarnGuidance(condition, authority, next) {
 ...
 */
export function checkContextInjectionConfig(config, id = "context-injection") {
    if (config === null)
        return null;
    if (!("context_injection" in config))
        return null;
 ...
 */
function alertMirrorNextTarget(block, resolved) {
    if (!resolved.enabled)
        return "alert_mirror.min_severity";
...
export function checkAlertMirrorConfig(config) {
...
export function checkServiceConfig(config) {
    ...
    if (notInstalled.length > 0) {
        return {
            id: "services",
            status: "warn",
            message: withWarnGuidance(`Declared service target(s) not installed as a service: ${notInstalled.join(", ")}.`, AUTHORITY_SERVICES, `inspect services.transports.${notInstalled[0]} in ${LOCAL_CONFIG_PATH}.`),
        };
    }
    if (notRunning.length > 0) {
        return {
            id: "services",
            status: "warn",
            message: withWarnGuidance(`Installed service target(s) reported as not running: ${notRunning.join(", ")}.`, AUTHORITY_SERVICES, `inspect services.transports.${notRunning[0]} in ${LOCAL_CONFIG_PATH}.`),
        };
    }
    return {
        id: "services",
    ...
    if (isPlainRecord(automation)) {
        for (const key of SCHEDULER_AUTOMATION_CLASSES) {
            ...
            if (raw !== undefined && raw !== null && typeof raw !== "boolean") {
                return warn(`scheduler.automation.${key}`, `scheduler.automation.${key} is present but is not a boolean; ${SCHEDULER_CLASS_LABELS[key]} automation resolves disabled (fail closed).`);
            }
        }
    }
