 ...
 */
export function mergeEffectivePackages(manifests) {
    const map = new Map();
 ...
 */
export function diagnosePackages(effective, localPackages, packageInstalled, blockedPackages) {
    const localSet = new Set(localPackages);
    const blockedSet = new Set(blockedPackages ?? []);
    return effective.map((pkg) => {
...
function formatSource(source) {
    if (source.kind === "shared")
        return "shared (packages.yml)";
    if (source.kind === "group")
        return `group/${source.group} (agent-groups/${source.group}/packages.yml)`;
 ...
 */
export function formatPackageList(effective, agentName) {
 ...
 */
export function formatPackageExplain(effective, agentName) {
 ...
 */
export function formatPackageDoctor(diagnosed, agentName) {
