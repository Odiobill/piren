...
function assertValidProjectName(project) {
...
function projectIndexPath(project) {
...
export async function projectAppendLog(options) {
    assertValidProjectName(options.project);
    ...
    if (!("bytesAppended" in result)) {
        throw new Error(`Project log append was queued instead of written authoritatively: ${result.reason}`);
    }
    return {
        path: result.path,
...
export async function decisionRecord(options) {
    assertValidProjectName(options.project);
    ...
    if (options.consequences !== undefined)
        renderOptions.consequences = options.consequences;
    if (options.alternatives !== undefined)
        renderOptions.alternatives = options.alternatives;
...
function assertValidSkillCandidateName(name) {
...
export async function projectUpdateHandoff(options) {
    assertValidProjectName(options.project);
...
export async function runbookWrite(options) {
    assertValidProjectName(options.project);
...
export async function skillCandidateWrite(options) {
