...
export function resolveVaultPath(vaultRoot, inputPath) {
...
async function assertVaultAvailable(vaultRoot) {
    let metadata;
    try {
...
async function queueWrite(options) {
...
export function createVaultTools(options) {
    ...
    return {
        ...
        async vaultPatch(path, oldText, newText) {
            if (oldText === "") {
        ...
        async vaultAppendLog(path, entry) {
            const resolved = resolveVaultPath(vaultRoot, path);
