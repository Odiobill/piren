...
import { createInteractiveFallbackIncident, recordInteractiveFallbackEvent, settleInteractiveFallbackIncident, } from "./interactive-model-fallback.js";
...
import { createAlertMirrorState, formatAlertMirrorDeliveries, mirrorStewardAlert, resolveAlertMirrorConfig, } from "./alert-mirror.js";
...
function scriptCronTimeoutMs(env) {
...
function extractInteractiveModelId(ctx) {
...
function splitInteractiveModelId(modelId) {
    const slash = modelId.indexOf("/");
    if (slash <= 0 || slash === modelId.length - 1)
        return null;
    return { provider: modelId.slice(0, slash), id: modelId.slice(slash + 1) };
}
function extractInteractivePrompt(event) {
    if (!isRecord(event) || typeof event.prompt !== "string" || event.prompt === "")
        return null;
    return event.prompt;
}
function extractInteractiveImages(event) {
    return isRecord(event) && Array.isArray(event.images) ? [...event.images] : [];
}
function isInteractiveSessionIdle(ctx) {
    if (!isRecord(ctx) || typeof ctx.isIdle !== "function")
        return false;
    try {
...
function buildReviewChildPiArgs(context, prompt) {
    return [
        "-p",
        "--no-session",
        "--no-extensions",
...
function jobIsDue(job, now) {
    const options = { schedule: job.schedule, now };
    if (job.lastRun !== undefined)
        options.lastRun = job.lastRun;
    return isScheduleDue(options);
}
function contextPrompt(context, skills = []) {
    const lines = [
        "# Piren Context",
...
export default async function pirenExtension(pi, testOptions = {}) {
    ...
    let pendingInteractiveContinuation;
    let interactiveExplicitModelSelected = false;
    let automaticInteractiveModelId;
    ...
    if (reviewLoop.enabled && typeof reviewExec === "function") {
        ...
        pi.on("turn_end", async (_event, ctx) => {
            ...
            if (reviewInProgress)
                return;
            if (turnsSinceReview < reviewLoop.intervalTurns)
                return;
            turnsSinceReview = 0;
            ...
            void execReview("pi", buildReviewChildPiArgs(context, prompt), { timeout: reviewLoop.timeoutMs })
                ...
                if (result.code !== 0 || !output || output === "Nothing to promote.")
                    return;
                const notify = ctx?.ui?.notify;
                if (typeof notify === "function") {
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.severity !== undefined)
                    alertOptions.severity = params.severity;
                if (params.notify !== undefined)
                    alertOptions.notify = params.notify;
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.requires_approval !== undefined)
                    taskOptions.requiresApproval = params.requires_approval;
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.result !== undefined)
                    updateOptions.result = params.result;
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.stale_after_ms !== undefined)
                    claimOptions.staleAfterMs = params.stale_after_ms;
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (skill.description)
                text.push(`Description: ${skill.description}`);
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.consequences !== undefined)
                    adrOptions.consequences = params.consequences;
                if (params.alternatives !== undefined)
                    adrOptions.alternatives = params.alternatives;
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.scope !== undefined)
                    candidateOptions.scope = params.scope;
        ...
        async execute(_toolCallId, params) {
            try {
                ...
                if (params.links !== undefined)
                    updateOptions.links = params.links;
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            try {
                const listOptions = { vaultRoot: context.vaultRoot, agentName: context.agentName };
    ...
    if (parsedFallback.ok && parsedFallback.present) {
        pi.on("model_select", async (event) => {
            ...
            if (!isRecord(event) || event.source === "restore")
                return;
        ...
        const interactiveEvidenceEvents = [
            "message_start",
            "message_update",
            "message_end",
            "turn_end",
        ...
        pi.on("agent_settled", async (event, ctx) => {
            ...
            interactiveIncident = undefined;
            if (incident === undefined || !parsedFallback.ok || !parsedFallback.present)
                return;
            ...
            const manualRecovery = (reason) => {
                if (typeof notify === "function")
                    notify(`[model fallback: manual recovery required (${reason}).]`, "warning");
            };
