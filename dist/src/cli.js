...
async function runTransportConfigureCommand(kind) {
...
try {
    if (command === "init") {
        ...
        if (wantsInteractive && process.stdin.isTTY) {
            ...
            // running.
            if (parsed.apply)
                console.log(formatPostSetupWorkbenchSuggestion());
        }
    }
...
const STARTER_SKILLS_USAGE = "Usage: piren skills seed --profile <name> [--vault-root <path>] [--dry-run] [--yes] | piren skills doctor [--profile <name>] [--vault-root <path>]";
...
async function runSkillsCommand(args) {
    ...
    const { join } = await import("node:path");
    const { parse: parseYaml } = await import("yaml");
    const { assertVaultRoot, applyStarterSeed, createRealStarterSkillsDeps, listStarterProfiles, parseStarterManifest, planStarterSeed, resolveStarterTemplatesDir, runStarterDoctor, validateProfileTemplates, } = await import("./starter-skills.js");
    const sub = args.positionals[0];
    ...
    async function loadManifest(profile) {
    ...
    if (plan.blockedByDuplicate) {
        for (const item of plan.items) {
            if (item.duplicateOverlay !== null)
                console.error(`  blocked: ${item.duplicateOverlay.reason}`);
        }
    ...
    for (const item of creates)
        console.log(`  create  ${item.targetPath}`);
    for (const item of conflicts)
        console.log(`  conflict (${stateLabel(item.state)}): ${item.targetPath}`);
