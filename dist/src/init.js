...
import { BASELINE_DIRECTIVE_SECTION, BASELINE_SKILL_NAME, createBaselineSkill, loadBaselineAssets, preflightBaselineSkill, probeRecognizedVault, } from "./init-baseline.js";
import { createRealStarterSkillsDeps, resolveStarterTemplatesDir } from "./starter-skills.js";
...
function defaultPirenEntityContent() {
...
function defaultOkfConceptContent() {
    const today = new Date().toISOString().slice(0, 10);
    return [
        "---",
        "type: Concept",
...
function defaultPirenVaultConceptContent() {
    const today = new Date().toISOString().slice(0, 10);
    return [
        "---",
        "type: Concept",
...
function defaultPirenAgentOperatingModelConceptContent() {
    const today = new Date().toISOString().slice(0, 10);
    return [
        "---",
        "type: Concept",
...
function buildStewardDirectivesContent(includeBaseline) {
    const lines = [
        "# Steward Directives",
        "",
...
export async function initVault(options) {
    ...
    const baseline = { directiveIncluded: false, skillCreated: false, warning: null };
    let baselineAssets = { ok: false, reason: "recognized existing vault" };
    ...
    // ADR-0043: the retired Rooms collaboration area is decommissioned — fresh
    // vaults no longer create the obsolete `collaboration/rooms/` directory.
    ...
    try {
        ...
        await writeNewFile(join(vaultRoot, "steward-directives.md"), buildStewardDirectivesContent(baseline.directiveIncluded), force, created);
        ...
        await writeNewFile(join(vaultRoot, "wiki", "entities", "piren.md"), defaultPirenEntityContent(), force, created);
