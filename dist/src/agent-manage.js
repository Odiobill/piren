...
// Pure list editing
// ---------------------------------------------------------------------------
export function addAllowedAgent(existing, agent) {
    if (existing.includes(agent))
        return [...existing];
    return [...existing, agent];
}
export function removeAllowedAgent(existing, agent) {
    return existing.filter((entry) => entry !== agent);
}
export function agentDirPath(vaultRoot, agentName) {
    return join(vaultRoot, "team", agentName);
}
// ---------------------------------------------------------------------------
// Config read/write (preserves unrelated keys)
// ---------------------------------------------------------------------------
function readConfigObject(existingConfig) {
 ...
 */
export function updateAllowedAgentsInConfig(existingConfig, nextAllowed) {
...
function allowedAgentsFromConfig(existingConfig) {
    const root = readConfigObject(existingConfig);
    return normalizeStringArray(root.allowed_agents);
}
export function planAddAgent(opts) {
...
export function planRemoveAgent(opts) {
    const dirPath = agentDirPath(opts.vaultRoot, opts.agentName);
    const currentAllowed = allowedAgentsFromConfig(opts.existingConfig);
...
export function planCloneAgent(opts) {
...
export async function executeAddAgent(opts) {
    const dirPath = agentDirPath(opts.vaultRoot, opts.agentName);
...
export async function executeCloneAgent(opts) {
    const sourceDir = agentDirPath(opts.vaultRoot, opts.sourceAgent);
    const targetDir = agentDirPath(opts.vaultRoot, opts.targetAgent);
...
export async function executeRemoveAgent(opts) {
    const dirPath = agentDirPath(opts.vaultRoot, opts.agentName);
...
//# sourceMappingURL=agent-manage.js.map