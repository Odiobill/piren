 ...
 */
export function conversationGenericId(now, suffix) {
    if (!CONVERSATION_GENERIC_SUFFIX_PATTERN.test(suffix)) {
        throw new Error(`Invalid conversation id suffix: must be exactly ${CONVERSATION_GENERIC_SUFFIX_HEX_LENGTH} lowercase hexadecimal characters.`);
    }
 ...
 */
export function randomConversationSuffix() {
...
export async function createConversation(options) {
    ...
    if (bytes === 0) {
        throw new Error(`Conversation already exists (${CONVERSATION_GENERIC_MAX_CANDIDATES} id candidates exhausted). Refusing to overwrite existing conversation evidence.`);
    }
    return {
        id,
