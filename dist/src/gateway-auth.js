 ...
 */
export function isLocalhostBind(hostname) {
 ...
 */
export function assertAuthGate(input) {
    if (input.token !== "")
        return;
    if (isLocalhostBind(input.hostname))
        return;
    throw new Error(`Auth token is required when binding to a non-localhost address (${input.hostname}). ` +
 ...
 */
export async function resolveGatewayToken(options) {
...
async function writeTokenFile(path, token) {
    const dir = join(path, "..");
