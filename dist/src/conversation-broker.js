 ...
 */
export function buildConversationAgentStartPrompt(input) {
    return [
        `You are participating in Piren conversation '${input.conversationId}' as agent '${input.agent}'.`,
 ...
 */
const CONVERSATION_AGENT_START_CONTEXT_METADATA = {
    truncated: false,
    selectedIds: [],
    omittedIds: [],
    selectedCount: 0,
...
export class ConversationBroker {
     ...
     */
    async startConversationAgentRun(input) {
        if (this.closed) {
            throw new Error("Conversation broker is closed.");
        }
        let conversation;
        ...
        try {
            return await this.executeConversationRun(run, buildConversationAgentStartPrompt({ conversationId: input.conversationId, agent: input.agent }), CONVERSATION_AGENT_START_CONTEXT_METADATA, done);
        }
        finally {
            this.activeRuns.delete(run.key);
