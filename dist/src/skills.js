 ...
 */
async function scanSkillsDir(dir, relBase) {
    let entries;
    try {
 ...
 * UTF-16 code-unit (lexical) order, so same-name collisions between groups
 * resolve the same way on every filesystem and host locale.
 ...
 * skill bodies.
 */
export function formatSkillCatalogForContext(skills) {
    if (skills.length === 0)
        return "";
    const lines = [
        "## Available Skills",
