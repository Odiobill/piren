...
import { GroupSettingsError, createNodeGroupSettingsDeps, listGroups, listVaultAgents, mutateGroup, readGroup, validateAllGroups, } from "./group-settings.js";
...
export class GatewayServer {
    ...
    stewardAlertJson(alert) {
        ...
        if (alert.resolvedAt !== undefined)
            result.resolved_at = alert.resolvedAt;
        return result;
    }
     ...
     */
    async handleGroupsValidation(res) {
        const root = this.groupsRoot();
    ...
    async handleGroupsAction(req, res) {
        ...
        try {
            ...
            if (error instanceof GroupSettingsError) {
                const statusByCode = { conflict: 409, "not-found": 404, exists: 409, invalid: 400, io: 500 };
        ...
        if (agentScope !== undefined) {
            for (const names of [agentScope.inbox_tasks, agentScope.agent_cron, agentScope.script_cron]) {
                ...
                for (const name of names) {
                    ...
                    seen.add(name);
                    if (!this.runnableAgents.includes(name))
                        nonRunnable += 1;
                }
                if (nonRunnable > 0) {
