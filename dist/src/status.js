...
async function checkVaultAvailability(vaultRoot) {
    let metadata;
    try {
...
async function inspectLocalCache(localCacheDir) {
    try {
...
export async function buildPirenStatusReport(options) {
...
export function formatPirenStatusReport(report) {
    const lines = [
        "Piren status",
