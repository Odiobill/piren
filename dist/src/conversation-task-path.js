...
const CANDIDATE_PATTERN = /(?:^|[\s"'`(\[{<])(team\/[a-z][a-z0-9-]*\/inbox\/[0-9]{8}T[0-9]{9}Z-[a-z0-9]+(?:-[a-z0-9]+)*\.md)(?![\w.-])/g;
/** True when `value` is exactly one ordinary vault-relative inbox task path. */
export function isConversationInboxTaskPath(value) {
 ...
 */
export function extractConversationTaskPath(text) {
    const found = new Set();
...
//# sourceMappingURL=conversation-task-path.js.map