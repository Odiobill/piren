...
async function load(label, agentDir, configPath, env = {}) {
    const pi = fakePi();
    await extension(pi, { cliAgentDir: agentDir, env, configPath });
...
async function main() {
    ...
    try {
        ...
        await writeFile(join(fixture.vault, "skills", "check-disk.md"), [
            "---",
            "name: check-disk",
        ...
        await writeFile(join(fixture.vault, "team", "thor", "skills", "deploy.md"), [
            "---",
            "name: deploy",
        ...
        const skillStatusNotifications = [];
        ...
        if (!skillBeforeStart)
            throw new Error("skills before_agent_start handler not found");
        const skillResult = await skillBeforeStart();
        const skillContent = skillResult.message.content;
        ...
        if (read.isError || !read.content[0].text.includes("Keep the spike"))
            throw new Error("vault_read smoke failed");
        ...
        if (write.isError)
            throw new Error(`vault_write smoke failed: ${write.content[0].text}`);
        const written = await readFile(join(fixture.vault, "team", "thor", "logs", "spike-test.md"), "utf8");
        if (!written.includes("Piren vault_write smoke passed"))
            throw new Error("written file did not contain expected content");
        ...
        if (list.isError || !list.details.entries.some((entry) => entry.path === "team/thor/logs/spike-test.md")) {
        ...
        if (patch.isError)
            throw new Error(`vault_patch smoke failed: ${patch.content[0].text}`);
        const patched = await readFile(join(fixture.vault, "team", "thor", "logs", "spike-test.md"), "utf8");
        if (!patched.includes("Piren vault_patch smoke passed"))
            throw new Error("patched file did not contain expected content");
        ...
        if (appendLog.isError)
            throw new Error(`vault_append_log smoke failed: ${appendLog.content[0].text}`);
        const activity = await readFile(join(fixture.vault, "team", "thor", "logs", "activity.md"), "utf8");
        if (!activity.includes("Smoke append-log passed."))
            throw new Error("activity log did not contain expected content");
        ...
        if (sessionSummary.isError)
            throw new Error(`session_write_summary smoke failed: ${sessionSummary.content[0].text}`);
        const summaryContent = await readFile(join(fixture.vault, sessionSummary.details.path), "utf8");
        if (!summaryContent.includes("Piren session summary smoke passed."))
            throw new Error("session summary did not contain expected content");
        ...
        if (alert.isError)
            throw new Error(`flag_steward smoke failed: ${alert.content[0].text}`);
        ...
        await writeFile(join(fixture.vault, "Projects", "Piren", "index.md"), [
            "---",
        ...
        if (logResult.isError)
            throw new Error(`project_append_log smoke failed: ${logResult.content[0].text}`);
        ...
        if (adrResult.isError)
            throw new Error(`decision_record smoke failed: ${adrResult.content[0].text}`);
        ...
        if (handoffResult.isError)
            throw new Error(`project_update_handoff smoke failed: ${handoffResult.content[0].text}`);
        ...
        if (runbookResult.isError)
            throw new Error(`runbook_write smoke failed: ${runbookResult.content[0].text}`);
        ...
        if (candidateResult.isError)
            throw new Error(`skill_candidate_write smoke failed: ${candidateResult.content[0].text}`);
        ...
        if (wikiConceptResult.isError)
            throw new Error(`wiki_update_concept smoke failed: ${wikiConceptResult.content[0].text}`);
        ...
        if (wikiEntityResult.isError)
            throw new Error(`wiki_update_entity smoke failed: ${wikiEntityResult.content[0].text}`);
        ...
        const reviewPi = fakePi();
        const reviewExecCalls = [];
        reviewPi.exec = async (command, args, options) => {
            reviewExecCalls.push({ command, args, options });
            return { code: 0, stdout: "Nothing to promote." };
        };
        await extension(reviewPi, {
        ...
        await reviewHandler({ message: { role: "assistant", content: "done" } }, {
            sessionManager: {
                getBranch: () => [
        ...
        await writeFile(join(fixture.vault, "cron", "jobs", "smoke-digest.md"), [
            "---",
            "id: smoke-digest",
            'agent: "thor"',
            'schedule: "0 7 * * *"',
        ...
        if (cronList.isError || !cronList.details.jobs.some((j) => j.id === "smoke-digest")) {
        ...
        if (cronClaim.isError)
            throw new Error(`cron_claim smoke failed: ${cronClaim.content[0].text}`);
        ...
        if (cronRecord.isError)
            throw new Error(`cron_record_run smoke failed: ${cronRecord.content[0].text}`);
        ...
        if (cronRuns.isError || !cronRuns.details.runs.some((r) => r.jobId === "smoke-digest")) {
        ...
        await writeFile(join(fixture.vault, "cron", "jobs", "smoke-script.md"), [
            "---",
            "id: smoke-script",
            'agent: "thor"',
            'schedule: "30m"',
        ...
        if (sentTask.isError)
            throw new Error(`send_to_agent smoke failed: ${sentTask.content[0].text}`);
        ...
        if (updatedTask.isError)
            throw new Error(`task_update_status smoke failed: ${updatedTask.content[0].text}`);
        ...
        if (inboxList.isError)
            throw new Error(`inbox_list smoke failed: ${inboxList.content[0].text}`);
        ...
        if (claimedTask.isError)
            throw new Error(`task_claim smoke failed: ${claimedTask.content[0].text}`);
        ...
        try {
            ...
            if (error instanceof Error && error.message === "task_claim left original task path in place")
                throw error;
        }
        ...
        if (reclaimedTask.isError)
            throw new Error(`stale task_claim recovery smoke failed: ${reclaimedTask.content[0].text}`);
        ...
        try {
            ...
            if (error instanceof Error && error.message === "stale task_claim recovery left old claim path in place")
                throw error;
        }
        ...
        if (!outside.isError || !outside.content[0].text.match(/outside vault/i))
            throw new Error("outside write was not rejected");
        ...
        if (restartRead.isError)
            throw new Error("restart read failed");
        ...
        if (restartWrite.isError)
            throw new Error("restart write failed");
        ...
        const fakePackageResolver = (name) => "/fake/node_modules/" + name + "/dist/index.js";
        ...
        const extensionFlags = pkgCommand.args.reduce((acc, arg, i) => {
            if (arg === "--extension")
                acc.push(pkgCommand.args[i + 1] ?? "");
            return acc;
        }, []);
        ...
        const packagesCheck = doctorReport.checks.find((c) => c.id === "packages");
        ...
        const missingResolver = (name) => {
            if (name === "@piren/git-tools")
        ...
        const missingPackagesCheck = missingDoctorReport.checks.find((c) => c.id === "packages");
        ...
        if (workerTask.isError)
            throw new Error(`worker send_to_agent smoke failed: ${workerTask.content[0].text}`);
        ...
        try {
            ...
            if (startRes.status !== 200)
                throw new Error(`gateway start HTTP ${startRes.status}`);
            ...
            for (const block of sseText.split("\n\n")) {
                ...
                if (evt === "token")
                    tokens.push(JSON.parse(data).text);
                else if (evt === "done")
                    sawDone = true;
            }
            if (!sawDone)
                throw new Error("gateway SSE stream did not end with done");
            if (tokens.join("") !== "Hello")
                throw new Error(`gateway streamed text mismatch: ${tokens.join("")}`);
        }
        finally {
            await gateway.close();
        }
        ...
        try {
            ...
            // List the vault root.
            const listRes = await fetch(`http://${browserHandle.hostname}:${browserHandle.port}/api/vault/list`);
            if (listRes.status !== 200)
                throw new Error(`vault browser list HTTP ${listRes.status}`);
            ...
            const readRes = await fetch(`http://${browserHandle.hostname}:${browserHandle.port}/api/vault/read?path=steward-directives.md`);
            if (readRes.status !== 200)
                throw new Error(`vault browser read HTTP ${readRes.status}`);
            ...
            const graphRes = await fetch(`http://${browserHandle.hostname}:${browserHandle.port}/api/vault/graph`);
            if (graphRes.status !== 200)
                throw new Error(`vault graph HTTP ${graphRes.status}`);
        ...
        let askTokens = "";
        const askText = await askAgent({ command: process.execPath, args: [fakePiScript], cwd: process.cwd(), env: process.env }, "Hello", (token) => { askTokens += token; });
        ...
        try {
            ...
            if (modelsRes.status !== 200)
                throw new Error(`models HTTP ${modelsRes.status}`);
            const modelsBody = (await modelsRes.json());
            if (modelsBody.models.length === 0)
            ...
            if (stateRes.status !== 200)
                throw new Error(`state HTTP ${stateRes.status}`);
            const stateBody = (await stateRes.json());
            if (stateBody.sessionId !== "fake-session")
                throw new Error("state sessionId mismatch");
            // POST /api/chat/model
            ...
            if (setModelRes.status !== 200)
                throw new Error(`set model HTTP ${setModelRes.status}`);
            ...
            if (thinkingRes.status !== 200)
                throw new Error(`set thinking HTTP ${thinkingRes.status}`);
            // GET /api/chat/agents
            const agentsRes = await fetch(`http://${modelHandle.hostname}:${modelHandle.port}/api/chat/agents`);
            if (agentsRes.status !== 200)
                throw new Error(`agents HTTP ${agentsRes.status}`);
            ...
            if (switchRes.status !== 200)
                throw new Error(`switch HTTP ${switchRes.status}`);
            ...
            const agentsAfterBody = (await agentsAfter.json());
            if (agentsAfterBody.current !== "thor")
                throw new Error("agent did not switch to thor");
        }
        finally {
        ...
        try {
            ...
            if (startRes.status !== 200)
                throw new Error(`start HTTP ${startRes.status}`);
            // Send a steer.
            ...
            if (steerRes.status !== 200)
                throw new Error(`steer HTTP ${steerRes.status}`);
            // Send a follow-up.
            ...
            if (followRes.status !== 200)
                throw new Error(`follow_up HTTP ${followRes.status}`);
            ...
            if (badModeRes.status !== 400)
                throw new Error(`bad mode HTTP ${badModeRes.status}`);
            ...
            if (approveStartRes.status !== 200)
                throw new Error(`approve-start HTTP ${approveStartRes.status}`);
            ...
            if (approveRes.status !== 200)
                throw new Error(`approve HTTP ${approveRes.status}`);
        }
        finally {
        ...
        try {
            ...
            if (infoRes.status !== 200)
                throw new Error(`auth info HTTP ${infoRes.status}`);
            const infoBody = (await infoRes.json());
            if (!infoBody.authRequired)
                throw new Error("auth info did not report authRequired=true");
            // Unauthenticated request is rejected.
            const unauthRes = await fetch(`http://${authHandle.hostname}:${authHandle.port}/api/chat/state`);
            if (unauthRes.status !== 401)
                throw new Error(`unauthenticated state HTTP ${unauthRes.status}`);
            // Wrong token is rejected.
            const wrongRes = await fetch(`http://${authHandle.hostname}:${authHandle.port}/api/chat/state`, {
                headers: { authorization: "Bearer " + "wrong" },
            });
            if (wrongRes.status !== 401)
                throw new Error(`wrong-token state HTTP ${wrongRes.status}`);
            ...
            if (okRes.status !== 200)
                throw new Error(`authenticated state HTTP ${okRes.status}`);
        }
        finally {
        ...
        try {
            ...
            const openInfoBody = (await openInfo.json());
            if (openInfoBody.authRequired)
                throw new Error("open server reported authRequired=true");
            const openState = await fetch(`http://${openHandle.hostname}:${openHandle.port}/api/chat/state`);
            if (openState.status !== 200)
                throw new Error(`open state HTTP ${openState.status}`);
        }
        finally {
        ...
        try {
            ...
            if (apiRes.status !== 200)
                throw new Error(`api over static HTTP ${apiRes.status}`);
        }
        finally {
        ...
        await writeFile(join(sessionVault, "team", "thor", "sessions", "20260623T090000Z-smoke.md"), [
            "---",
        ...
        try {
            ...
            if (sessionsRes.status !== 200)
                throw new Error(`sessions HTTP ${sessionsRes.status}`);
            ...
            if (resumeRes.status !== 200)
                throw new Error(`resume HTTP ${resumeRes.status}`);
            const resumeBody = (await resumeRes.json());
            if (resumeBody.cancelled)
                throw new Error("resume reported cancelled unexpectedly");
            ...
            if (abortStartRes.status !== 200)
                throw new Error(`abort-start HTTP ${abortStartRes.status}`);
            const abortRes = await fetch(`http://${sessionHandle.hostname}:${sessionHandle.port}/api/chat/abort`, {
                method: "POST",
            });
            if (abortRes.status !== 200)
                throw new Error(`abort HTTP ${abortRes.status}`);
            ...
            if (badResumeRes.status !== 400)
                throw new Error(`bad resume HTTP ${badResumeRes.status}`);
        }
        finally {
