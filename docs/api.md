...
- `GET /api/conversations/<id>/workflow-budgets` — the Conversation-level budget read behind the Details modal's Workflow budget section: a bounded list of the newest durable workflow roots in durable sequence order, each with the same per-root facts as above (fixed chain depth 3, `warnings`, `omittedWarnings`), plus `total` (all durable workflow roots, including any beyond the response bound) and `omitted` (roots beyond the bound) so truncation is never silent, plus the server-resolved per-agent association map (each locally known agent → its associated root or `null`). The client renders the roots as a list and never selects a "current" root on its own; there is no per-agent budget. Unknown conversation is 404; failure mappings follow the family.
