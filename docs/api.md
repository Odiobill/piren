...
Local runnable-agent roster (Rooms product decommission rename of the old `/api/room-agents` route, which is removed without alias):
...
The retired Rooms product (room routes, room broker, `room_mention` extension tool, room web surface) is decommissioned with no compatibility layer.
