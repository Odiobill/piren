...
- `GET /api/settings/telegram` and `GET /api/settings/discord`: the fully redacted transport projection: `200 {available: true, telegram|discord: {...}}` with a token `configured` boolean, the stored non-secret allowlist identifier values returned for editable prefill (alongside bounded counts), default agent, and feedback state (never a token, fingerprint, raw config, or unknown field), or `200 {available: false, reason}` with a bounded non-secret reason when the config is missing or malformed.
- `GET /api/settings/scheduler`: the redacted scheduler projection: the three closed automation classes (the sole ordinary execution gates), the bounded retired-key state (`legacyMasterGate`: `absent`, `ignored`, or `gated`), and the editable poll/stale/concurrency/device values. Read-only: no tick, claim, spawn, or migration.
...
Agent Groups (typed vault-owned workflows over `agent-groups/<group>/config.yml`; same Bearer gate and closed-inventory discipline as every Settings route; requires the gateway's vault root):

- `GET /api/settings/groups`: bounded typed group summaries: `200 {available: true, groups: [{name, revision}]}`. Never raw YAML.
- `GET /api/settings/groups/validation`: read-only cross-group report reusing the `piren group validate` categories (`missing-config`, `dangling-fallback`, `missing-agent-dir`, and the `duplicate-across-groups` info note). Response: `200 {available: true, issues: [{group, kind, severity, message}]}`. It writes nothing.
- `GET /api/settings/groups/<name>`: the modelled detail for one group plus the full vault-defined roster: `{available: true, group: {name, revision, agents, fallbackOrder, findings}, roster: [{name, locallyRunnable}]}` where `locallyRunnable` derives from this gateway's resolved runnable set. An unknown group is `200 {available: false, reason}`; an invalid name is a bounded 400.
- `POST /api/settings/groups`: one closed typed action: `{action: "create" | "add-agent" | "remove-agent" | "fallback-set", group, expectedRevision, agent?, candidates?, confirm?}`. Unknown fields are a 400. Create, remove-agent, and fallback-set require `confirm: true`; add-agent accepts only names that exist as vault-defined `team/<agent>/` identities (a non-vault name is a bounded 400). Writes are atomic with a source-revision check preserving unknown top-level YAML fields: a config changed since it was read is a bounded `409` and nothing is clobbered. Success returns exactly `200 {wrote: true}`.
...
- `POST /api/conversations/start-peer`: peer-audience Conversation start. Body is exactly `{peers}`: an array of 2 to 8 agent names. Validation is fail-before-persist and atomic: an extra key, non-array value, or wrong cardinality is a bounded 400; blank entries, name-grammar violations, duplicates (duplicates are rejected, never collapsed), and non-string entries are a bounded 400 that identifies positions and reason classes only, never echoing submitted values. The whole audience is then validated against the gateway-resolved local runnable set before anything is written; any non-runnable member fails the whole request with a bounded 400 reporting only a count (`N requested peer(s) are not in the local runnable set.`), never names or indexes. On success the gateway creates exactly one open conversation whose initial audience is the validated set sorted ascending byte-wise (display stability only, never a lead, rank, or turn order) under an immutable generic cardinal title, appends exactly one system-authored `conversation_start_requested` origin event after the manifest, publishes that exact committed record to the scoped SSE stream, and returns `201 {conversation, event}` with no `dispatch` field. The route has zero broker contact: it produces no greeting, queue activity, run evidence, retry, dedupe, or membership change beyond the initial audience. Later, an explicit steward message's gateway-validated `@`-mention dispatches an existing audience member as-is, or, when it names a previously absent locally runnable agent, adds that agent to the audience before dispatch (same validation as message append); the start route itself performs no dispatch.
...
- `GET /api/conversations/<id>/agents/<agent>/workflow-status` — the W3 per-agent status read for one exact conversation × agent pair. The broker resolves both facts independently: `run_active` is a broker-authoritative snapshot of whether a run is active for that exact pair right now (including agent-first runs with no workflow), and the workflow association is either `workflow: null` (truthful "no association" — never fabricated) or a server-associated workflow view carrying `root_event_id`, `association: "active-run"|"latest-run"`, `base`/`effective` dimensions, `consumed`, `worstPairOccurrences`, the `low`/`exhausted` classifications, bounded per-root `warnings`, and `omittedWarnings` (count of same-root ignored facts beyond the warning cap). The browser provides no root, association, or busy claim; it renders these facts only. Unknown conversation or agent is 404; unauthenticated requests are 401; a malformed id keeps the family's 400 behavior; a durable read that cannot be truthfully served is a bounded non-secret 500. No SSE frame is added for this read.
- `GET /api/conversations/<id>/workflow-budgets` — a bounded Conversation-level inspection read of the newest durable workflow roots in durable sequence order, each with the same per-root facts as above (fixed chain depth 3, `warnings`, `omittedWarnings`), plus `total` (all durable workflow roots, including any beyond the response bound) and `omitted` (roots beyond the bound) so truncation is never silent, plus the server-resolved per-agent association map (each locally known agent → its associated root or `null`). It is not the Context telemetry popup's editor source: that uses the exact-pair workflow-status association, so the browser never chooses a "current" root from this list. There is no per-agent budget. Unknown conversation is 404; failure mappings follow the family.
- `POST /api/conversations/<id>/workflow-budget` — the steward update. Body is exactly `{root_event_id, edges?, rework_rounds?, expected_effective: {edges, reworkRounds}}`: at least one dimension, both absolute integer targets that must strictly raise within the fixed caps (edges 24, rework rounds 6; the chain depth is fixed at 3) — there is no clamping, no unlimited value, no refill, and no configuration override; the gateway is the sole validation authority and an invalid, non-raising, or out-of-cap request is a bounded 400 naming the failed rule. `expected_effective` is a compare-and-set guard: a mismatch is a bounded 409 carrying the current effective view, so two stewarded clients can never silently overwrite each other; archived conversations and genuine lock contention are also 409. Unknown root or conversation is 404; unauthenticated requests are 401; a bounded non-secret 500 covers a durable failure after an accepted write. The update persists one additive durable budget event and never dispatches, approves, aborts, retries, or changes a pending handoff gate.
