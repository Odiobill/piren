...
- `POST /api/conversations/<id>/attach` — the C1/runnable-roster-gated active attach (C3-A). Reads the durable manifest and applies the accepted C1 active gate against the gateway's resolved runnable set. Returns `200 {conversation, attached: true, gate: {ok: true, missing: [], malformed: []}}` when every durable audience member is locally runnable (including an empty audience); returns `409 {error, attached: false, gate: {ok: false, missing, malformed}}` with a non-secret message when a durable member is missing or malformed, in which case the conversation stays visibly read-only inspection. The route is **stateless**: no vault write, membership update, broker dispatch, Pi client/session creation, live subscription, queue, retry, or persistent active-conversation state. An absent conversation is 404; unauthenticated requests are 401. Archive/reopen and conversation approval/abort routes remain later, separately gated slices.
