...
The generated unit's `ExecStart` runs the resolved Piren command directly:

```text
ExecStart=/path/to/piren gateway --vault-root /path/to/vault --agent piren
```
...
service lifecycle integration are all available.
