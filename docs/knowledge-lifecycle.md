...
raw event / task / session
  ...
  -> project log entry
  -> project status / current project notes / handoff update
...
This read-only correction checker uses Hermes-style heuristics and suggests the minimum visible vault artifact to use next. It only advises. It does not mutate hidden memory, create SQLite state, or run silent background writes.
...
Current project documents and decision records have higher authority than old raw traces.
