...
- composer interlock: while the selected conversation has authoritative live agent work or a pending steward-scoped approval, the composer is interlocked — the textarea becomes read-only with a visible reason and the submit and policy controls disable; an unsent draft is preserved byte-for-byte and restored editable when the work settles. A submitted message always clears from the composer immediately, before the send request resolves, and never reappears as a read-only acknowledgement — its durable timeline event is the only evidence of the send. If a send fails while a real interlock is active, the exact failed draft is preserved inside that read-only interlock (it never bypasses active-run/approval protection) and becomes editable only when the interlock clears; without an active interlock a failed send restores the exact draft immediately as editable with a bounded error. Interlock never moves focus, never scrolls, never fetches, and never mutates anything, and a selection change discards it
- compact per-agent **Context cards** as the last row below the composer on the active surface: each card is one keyboard-operable button with a clearly labelled Context progress bar and a truthful session-only state — a real measured usage percent rendered with exactly two decimals (a truthful zero is `0.00%`), `temporarily unavailable after compaction`, `no context window information`, `no live session`, or `no telemetry yet` — and unknown or unavailable states never render a number. Activating a card opens one focus-managed details popup for that exact conversation × agent pair (agent, truthful state, context tokens/window/percent when present, model, thinking level, and auto-compaction — never session ids, paths, token totals, cost, or transcript content); its explicit Refresh control is the only telemetry read (one authenticated GET per click), and opening, mounting, selection, stream events, reconnects, and timers never fetch. Context telemetry is session-only: it lives in browser memory for the open Workbench lifetime and survives Conversation/agent switching by rehydrating the exact `conversation × agent` observation from that memory (never a fetch); it clears on page reload, Workbench unmount, and token-loss/typed-401 handover, and is never persisted or reconstructed from history. A rehydrated compact card keeps the SAME concise presentation (no stale label); ONLY the telemetry details popup identifies rehydrated data as `Last observed <time>` until a fresh live frame or an explicit Refresh for that exact pair supersedes it
...
- conversation run deadline: every Conversation agent run has a hard 60-minute (3600-second) deadline by default; a run that exceeds it settles with the durable `timed_out` terminal while the scoped abort control stays available — there is no retry, fallback, reroute, or re-dispatch. The optional vault-root `workbench.yml` key `conversation.run_timeout_seconds` (whole number `1..3600`) can shorten the deadline for every gateway process using that vault; see [Configuration](./configuration.md#workbench-config-workbenchyml). It is read once at gateway startup and applied on restart; malformed or out-of-range values log one bounded warning and fall back to the default. Pi command acknowledgements keep their own shorter bounds, independent of the run deadline
...
The Dashboard also offers a distinct peer-audience start beside the ordinary single-agent start. In peer mode you explicitly select two to eight locally runnable agents and submit one creation request over the authenticated `POST /api/conversations/start-peer` route. The gateway persists that initial audience as one durable Conversation with its system-authored `conversation_start_requested` origin evidence, but it dispatches no agent: there is no greeting, no chosen lead or speaking order, and no bulk or fan-out messaging. The ordering of the initial audience is canonical display stability only, never a rank or turn order. A member joins the live work only later, through an explicit steward message whose `@`-mention the gateway alone parses and validates against the local runnable set; the browser never derives recipients from text. If the result of a start request is unclear (for example a network failure), the Workbench permits only an explicit durable list refresh, never an automatic retry.
...
The Workbench also exposes Conversation approval/abort controls over the delivered routes. A scoped live `approval` SSE frame (bounded `{conversationId, agent, requestId, method, payload}` for exactly the `confirm`, `select`, and `input` Pi UI methods) on a selected active conversation renders a non-modal approval card with Confirm / Cancel (and a labeled input for select/input requests); submitting sends exactly one of `confirmed`, `value`, or `cancelled` with the request id to `POST /api/conversations/<id>/approve`, and a 200 is delivery acceptance only — the browser never fabricates an agent outcome. Abort lives on the temporary activity cards in the active conversation's history: a card appears ONLY while a valid broker `conversation_activity` working/text_delta/tool frame is current, identifies that exact broker-provided agent with its `working`/`typing` phase, may show a BOUNDED transient streamed tail (at most the most recent 400 characters of at most 2,000 retained, as plain text with exactly one leading ellipsis when truncated — never Markdown-rendered, linkified, or announced) and at most five most-recent sanitized tool lines (tool name `[A-Za-z0-9 _:-]`, max 80 chars, plus exactly `started`/`completed`/`failed`; raw tool args/results/output/environment/secret-shaped fields are rejected fail-closed and never rendered), and its accessible labelled control sends the existing abort request (`POST /api/conversations/<id>/abort {agent}`) for that agent only — membership is never treated as active-run authority and no audience guess/history reconstruction/private reasoning occurs; the cards clear on the existing settled/durable terminal/reconnect/history/selection/malformed-frame cleanup and are never reconstructed from durable history, `get_messages`, telemetry, polling, timers, or storage. Cards appear ONLY on the active surface: read-only/archived/gate-rejected inspection shows no approval card, no abort, and no composer-equivalent control. Unknown/stale/already-settled approvals and bounded 400/404/409/500/network errors are shown with a manual Retry only — no hidden retry, queue, auto-approval, auto-abort, browser persistence, or client-side recipient/request-id derivation. Controls are run-scoped: a pending approval from an already-running run of an archived conversation remains answerable, while a fresh lifecycle re-gate clears any stale card. Approval frames are never durable conversation events and never appear in historic event reads.
...
The Workbench includes a full-page **Settings** module of typed, validated configuration workflows organized as three accessible tabs in fixed order: **This installation**, **Agent settings**, and **Agent groups**. Tab selection is component-local and in-memory only; nothing about the page persists to browser storage. It is not a generic YAML or filesystem editor: it exposes exactly the inventoried families below, each over its own bounded route, and everything else stays in the CLI or hand-edited config.

This installation tab (machine-local `~/.config/piren/config.yml`):

- **Telegram** and **Discord** transport blocks: bot token (write-only: never returned by any route, never rendered after save, and never placed in URLs, browser storage, logs, errors, or durable artifacts), non-secret allowlist values prefilled from the redacted read and replaced only on an explicit save, default agent, and feedback preference. The default-agent select offers the locally runnable roster; the server validates a saved value against that roster and refuses a non-runnable one with a bounded 400. A static bot-setup help control per transport opens an accessible modal with fixed steps: no fetch, no platform contact, no secret handling. Validation is structural only; saving never contacts the platform.
- **Scheduler**: the three closed automation classes (inbox tasks, agent cron, script cron), which are the sole ordinary execution gates, plus poll/stale intervals, concurrency, and device id. A retired-key legacy gate state renders as a read-only notice, disables the form, and refuses saves with guidance to `piren scheduler configure`, the only migration writer. Saving configuration never installs, starts, stops, or ticks the scheduler or any service.

Agent settings tab (vault-owned `team/<agent>/config.yml`): dashboard-style cards select one locally runnable agent; saving edits only durable future-launch preferences and never changes a live Pi session's model or thinking. The model-fallback declaration uses an ordered editor where each entry is a row with move-up/move-down/remove controls and the visible order is the saved order; saving one that leaves auto-switch enabled requires a separate explicit confirmation modal presenting the bounded same-session behavior. The context-injection option label is truthful (`Default (session_start_only)`: selecting Default writes nothing); explicit `per_turn` declarations and the `PIREN_CONTEXT_INJECTION` override still win.

Agent groups tab (vault-owned `agent-groups/<group>/config.yml`): typed workflows over the existing group semantics: list, show, create, add member, remove member, per-member ordered fallback set, and a read-only cross-group validation report. Writes are revision-checked and atomic: a config changed since it was read is refused with 409 rather than clobbered, and unknown top-level fields are preserved. Create, remove-member, and fallback-set require explicit confirmation modals before anything writes. Membership comes from the vault-defined `team/<agent>/` roster only; members that are not locally runnable are visibly marked "Not locally runnable" and are never actionable here. No group action mutates local runnable policy, assigns work, reroutes, or executes anything, and there is no arbitrary vault editing or shell-out.

Reads are redacted projections: tokens report only whether one is configured and are never returned by any route; non-secret allowlist identifiers come back as redacted editable values for prefill (alongside bounded counts); malformed config fails closed with a bounded non-secret reason. Writes are atomic (temp file, fsync, rename) with a source-revision check: a file changed between read and save is refused rather than clobbered, and any validation or I/O failure leaves the prior file byte-for-byte intact. A save re-serializes the whole file, so YAML comments in an edited block may be normalized away.
