...
- per-agent **Context cards** below the composer: session-only context usage with an explicit Refresh only, held in browser memory and cleared on reload; a card can additionally show one truthful workflow-status indicator (see [Workflow budgets and card status](#conversations))
...
- a focus-managed **Conversation details** modal with the editable title, metadata, the Archive/Reopen controls, and the Workflow budget section (root-scoped durable facts, explicit steward raises with compare-and-set, manual Retry only)
...
Workflow budgets (Conversation Details → **Workflow budget**): the section reads the authenticated bounded budget view once when the modal opens and lists the Conversation's durable workflow roots — each root shows its base/effective dimensions, consumed handoff edges, worst-pair usage, the fixed chain depth (3), and bounded warning facts; a Conversation with zero workflows shows no fabricated budget. Each root has an explicit steward **Save**: the browser posts the closed compare-and-set (CAS) body carrying the last-read effective values, so a concurrent save is refused with 409 and never silently overwritten. A changed target — including one beyond the display caps — is submitted unchanged and the gateway's bounded 400 is the truthful outcome; the caps shown in the field help are guidance, never a local clamp. On success the view is re-read before the navigator re-gates; a failed post-save re-read stays mounted with a manual Retry that retries only that re-read, never the accepted update, and every failure offers manual Retry only — there is no hidden retry, queue, polling, or browser persistence. Budgets are per-workflow facts for the associated agent; there is no per-agent budget, and they are never conflated with Pi context telemetry.

Per-agent card status: each active Conversation context card can show one compact labelled status beside its Context bar, derived only from the exact-pair status snapshot and validated live activity — never from typing indicators, presence, unread, transcripts, or agent names. The fixed precedence is red > yellow > busy > none: **red** means the agent's associated workflow budget is exhausted (action required — open the Conversation Details to extend or wind down); **yellow** means the associated workflow budget is low; **busy** means an exact conversation × agent run is active — including an agent-first run without an associated workflow — and is kept current between reads by the existing live activity frames; **none** renders no indicator at all. Red and yellow exist only with a server-provided workflow association. Statuses are read at explicit moments only — fresh Conversation selection/attach, durable reread, Details modal close, and after a budget update — never on a timer, SSE frame, or card activation, and there is no browser polling, storage, or automatic retry; a failed status read shows no fabricated indicator. Every non-none state carries an accessible text label (color and motion never carry meaning alone) and the busy indicator has a reduced-motion static equivalent; the indicator lives inside the existing card button semantics with no additional focusable control.
